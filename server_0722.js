...
const ledRelay = new Gpio(17, { mode: Gpio.OUTPUT })
...
async function detectImage(imagePath) {
	...
	try {
		...
		const tensor = tf
			.tensor3d(raw.data, [raw.height, raw.width, 4], 'int32')
			.slice([0, 0, 0], [-1, -1, 3])
...
function runCleaningSequence(type = 'auto') {
	...
	cleaningTimeouts = [
		setTimeout(() => {
			console.log('servoWrite(500)')
			servo.servoWrite(500)
		}, 0),
		setTimeout(() => {
			console.log('servoWrite(2500)')
			servo.servoWrite(2500)
		}, 2000),
		setTimeout(() => {
			console.log('servoWrite(1500)')
...
function turnOnLight() {
	ledRelay.digitalWrite(1) // 릴레이 ON
	console.log('[조명] 켜짐')
}

function turnOffLight() {
	ledRelay.digitalWrite(0) // 릴레이 OFF
	console.log('[조명] 꺼짐')
}
